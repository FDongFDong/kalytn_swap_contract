...
import "./interfaces/IPODCallee.sol";

contract PODPair is KIP7, KIP7Metadata, Math {
    ...
    uint112 private reserve0; // PODPair 에서 기록하는 token0의 잔고(balance는 실제잔고)
    uint112 private reserve1; // PODPair 에서 기록하는 token1의 잔고(balance는 실제잔고)
    uint32 private blockTimestampLast; // uses single storage slot, accessible via getReserves

    uint256 public price0CumulativeLast; // 다른 컨트랙트에서 price oracle로 사용하기위한 변수(public)
    uint256 public price1CumulativeLast; // 다른 컨트랙트에서 price oracle로 사용하기위한 변수(public)
    ...
    constructor() public KIP7Metadata("V2 Pair", "V2", 18) {}
    ...
    (this low-level function should be called from a contract which performs important safety checks)
        ...
        PODPair contract에서 기록하고 있는 token0, token1 자산의 양과 
    ...
    */
    function mint(address to) public returns (uint256 liquidity) {
        ...
        if (totalSupply() == 0) {
            ...
            liquidity = Math.min(
                amount0.mul(totalSupply()) / reserve0_,
    ...
    (this low-level function should be called from a contract which performs important safety checks)
        - swap: 임의의 ERC20 간의 토큰 교환 함수 (eth, klay 등은 wrapped 된 ERC-20으로 교환해주어야함)
                이 함수를 실행하기전에, PODPair의 token0 나 token1 주소에 교환하고자하는 토큰을 미리 넣고 
    ...
    */
    function swap(
        ...
        if (data.length > 0)
            IPODCallee(to).PODCall(msg.sender, amount0Out, amount1Out, data);
